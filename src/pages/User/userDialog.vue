<template>
  <div>
    <el-dialog :visible.sync="dialog.show" :title="dialog.title" width="500px">
      <el-form :model="form" :rules="dialog.add?rules1:rules2" :ref="dialog.ref" status-icon>
        <el-form-item label="用户名" prop="name" :label-width="formLabelWidth">
          <el-input v-model="form.name" placeholder="请输入用户名">{{dialog.row}}</el-input>
        </el-form-item>
        <el-form-item label="邮箱" prop="email" :label-width="formLabelWidth">
          <el-input v-model="form.email" placeholder="请输入邮箱"></el-input>
        </el-form-item>
        <el-form-item label="密码" prop="password" :label-width="formLabelWidth" v-if="dialog.add">
        ...
        <el-form-item label="操作人" prop="identity" :label-width="formLabelWidth">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="resetForm(dialog.ref)">取 消</el-button>
        <el-button type="primary" @click="addForm(dialog.ref)">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async _updateUser() {
      try {
        let res = await updateUser(this.dialog.id,this.form);
        this.$message({
          message: "更新成功",
          type: "success"
        });
